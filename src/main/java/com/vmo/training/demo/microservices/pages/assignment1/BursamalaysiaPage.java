...
public class BursamalaysiaPage extends WebUI {
	...
	public void getCounter(String value) {
		...
		if (getText("//ul[@class='nav nav-fill justify-content-end']/li/a/span").trim().equalsIgnoreCase(value)) {
			...
			click("//ul[@class='nav nav-fill justify-content-end']/li/a/span[@text()='" + value + "']");
			for (int i = 0; i < list.size(); i++) {
				Counter counter = new Counter();
				counter.stockCode = codes.get(i).getText().trim();
				counter.stockName = names.get(i).getText().trim();
